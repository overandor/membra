...
async def upload_photo(session_id: str, request: PhotoUploadRequest):
    ...
    detections = await vision_service.detect_objects(request.photo_data)
    session["detections"].extend(detections)
    return {"photo_id": photo_id, "status": "uploaded", "detections_count": len(detections)}
...
async def upload_frame(session_id: str, request: FrameUploadRequest):
    ...
    detections = await vision_service.detect_objects(request.frame_data)
    session["detections"].extend(detections)
    return {"frame_id": frame_id, "status": "received", "detections_count": len(detections)}
...
async def get_detections(session_id: str):
    if session_id not in sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    session = sessions[session_id]
    return {"detections": session["detections"]}
